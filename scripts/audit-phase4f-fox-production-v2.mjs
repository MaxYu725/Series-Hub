...
assert.ok(show, `${TARGET_TITLE} must be present in production after Phase 4F persistence repair`);
...
assert.match(String(show.networks || ""), /(^| · )FOX( · |$)/, "network projection must include FOX");
assert.ok(["airing", "upcoming", "planned"].includes(show.status), `unexpected catalog status: ${show.status}`);
assert.ok(Number(show.latest_season_number || 0) >= 2, "catalog must expose at least two existing seasons");
