...
if (process.env.CI === "true" && productionUrl) {
  test("production has a live TMDB catalog and prints the Phase 1B audit sample", async () => {
    ...
    for (const row of rows) {
      ...
      if (row.title_zh_hk) hkChinese += 1;
      if (row.title_zh_tw) twChinese += 1;
      if (row.title_zh_cn) cnChinese += 1;
    ...
    const audit = {
      ...
      chinese_title_coverage: {
        any: `${chineseAny}/${rows.length}`,
        hk: `${hkChinese}/${rows.length}`,
        tw: `${twChinese}/${rows.length}`,
        cn: `${cnChinese}/${rows.length}`
      },
