  ...
  try {
    const result = await env.DB.prepare(
      `SELECT
        ...
        s.tvmaze_synced_at,
        s.updated_at,
        (SELECT ta.title FROM title_aliases ta WHERE ta.show_id = s.id AND ta.season_id IS NULL AND ta.locale = 'zh' AND ta.region = 'HK' ORDER BY ta.is_preferred DESC, ta.id ASC LIMIT 1) AS title_zh_hk,
        (SELECT ta.title FROM title_aliases ta WHERE ta.show_id = s.id AND ta.season_id IS NULL AND ta.locale = 'zh' AND ta.region = 'TW' ORDER BY ta.is_preferred DESC, ta.id ASC LIMIT 1) AS title_zh_tw,
        (SELECT ta.title FROM title_aliases ta WHERE ta.show_id = s.id AND ta.season_id IS NULL AND ta.locale = 'zh' AND ta.region = 'CN' ORDER BY ta.is_preferred DESC, ta.id ASC LIMIT 1) AS title_zh_cn,
