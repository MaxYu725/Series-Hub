...
function replaceOnce(path, from, to) {
  ...
  if (first < 0) throw new Error(`Missing patch anchor in ${path}: ${from.slice(0, 120)}`);
  if (text.indexOf(from, first + from.length) >= 0) throw new Error(`Non-unique patch anchor in ${path}: ${from.slice(0, 120)}`);
...
replaceOnce(
  'test/phase10b1-korea-schedule-priority.test.js',
...
fs.writeFileSync('test/phase10e-korea-coverage.test.js', `import test from "node:test";
...
fs.writeFileSync('docs/PHASE10E_KOREA_COVERAGE.md', `# Phase 10E — Korean Catalog Coverage Expansion
